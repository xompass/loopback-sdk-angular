...
function buildScopeMethod(models, modelName, method, options, app) {
  ...
  var targetModelName = targetClass ?
  ...
  if (modelClass.scopes[scopeName] === undefined) {
    ...
    var methodName = method.name.replace('prototype.', '');
    var _swaggerMethods = app.models[modelName].sharedClass._swaggerMethods;
  ...
  var reverseModel = findModelByName(models, targetModelName);
